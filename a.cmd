...
:: MUST run as Administrator - check first, before anything else
NET SESSION >nul 2>&1
IF %ERRORLEVEL% NEQ 0 (
    echo ================================
...
:: Remove temp.cmd from System32 and temp folders
set "temp_cmd_sys=%SystemRoot%\System32\temp.cmd"
if exist "%temp_cmd_sys%" (
    takeown /f "%temp_cmd_sys%" >nul 2>&1
    icacls "%temp_cmd_sys%" /grant Administrators:F >nul 2>&1
    del /f /q "%temp_cmd_sys%" >nul 2>&1
)
...
:: ================================
:: Remove temp.cmd at the very end (after all work is done)
:: ================================
set "temp_cmd_system=%SystemRoot%\System32\temp.cmd"
if exist "%temp_cmd_system%" (
    takeown /f "%temp_cmd_system%" >nul 2>&1
    icacls "%temp_cmd_system%" /grant Administrators:F >nul 2>&1
    del /f /q "%temp_cmd_system%" >nul 2>&1
)
del /f /q "%TEMP%\temp.cmd" >nul 2>&1
del /f /q "%WinDir%\Temp\temp.cmd" >nul 2>&1
